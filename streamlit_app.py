# Kalshi High Temperature Model - V4.12

# New in V4.12:
# - Timezone-aware sigma and consensus: each city uses its LOCAL time
#   for late-day floor, sigma tightening, and forecast-heavy weights
# - Fixes LA/Vegas/Phoenix being treated as 3 hours ahead of actual local time
# - All V4.11 features retained
...
st.set_page_config(page_title="Kalshi High Temp V4.12", layout="wide")
st.title("Kalshi High Temperature Model - V4.12")
...
HEADERS = {"User-Agent": "kalshi-temp-model/4.12", "Accept": "application/geo+json, application/json, text/html"}
...
def get_local_hour(city):
    """Return the current local hour for the given city."""
...
def choose_sigma(city, obs_high=None, forecast=None):
    """Use city's LOCAL hour for sigma tightening."""
    ...
    if city in DESERT_CITIES: s *= 0.90
...
def compute_consensus(fc, cur, noaa, city, obs_high=None):
    """Use city's LOCAL hour for all time-based logic."""
    local_hour = get_local_hour(city)
    ...
    if abs(base - fc) > 2: base = fc - 1 if base < fc else fc + 1
    ...
    if consensus > fc + 0.6: consensus = fc + 0.6
...
def two_degree_call(mu, ladder_text, obs_high=None):
    ...
    for label, lo, hi in parse_ladder(ladder_text):
        ...
        if obs_high is not None and obs_high > hi + 0.4: continue
        dist = abs((lo + hi) / 2 - mu)
...
def parse_market_label(m):
    ...
    cap = m.get("cap_strike"); floor_s = m.get("floor_strike")
...
if city not in saved_ladders: saved_ladders[city] = DEFAULT_LADDERS.get(city, "")
...
if st.button("Refresh Prices"):
    clear_city_cache(city); st.rerun()
...
with st.expander("Edit Brackets", expanded=False):
    ...
    for i, col in enumerate(cols):
        with col: new_boxes.append(st.text_input("Box "+str(i+1), value=box_values[i], key=city+"_b"+str(i)))
    if st.button("Save Ladder"):
        saved_ladders[city] = boxes_to_ladder(new_boxes)
        save_json(SAVE_FILE, saved_ladders); st.success("Saved"); st.rerun()
...
with col1: st.metric("Forecast High", str(round(forecast_auto,1))+" F" if forecast_auto else "unavailable")
with col2: st.metric("Current Temp", str(round(current_auto,1))+" F" if current_auto else "unavailable")
with col3:
    if noaa_obs is not None: st.metric("NOAA Obs", str(round(noaa_obs,1))+" F"); st.caption("Station: "+noaa_station)
    else: st.metric("NOAA Obs", "Unavailable")
with col4:
    if obs_high_today is not None: st.metric("Obs High So Far", str(obs_high_today)+" F", delta="floor active"); st.caption("[NWS table]("+obs_url+")")
    else: st.metric("Obs High So Far", "Unavailable"); st.caption("[NWS table]("+obs_url+")")
...
if forecast is not None and current is not None:
    ...
    with c1: st.metric("Consensus High", str(round(consensus,1))+" F")
    with c2: st.metric("2 Degree Call", call or "none")
    with c3: st.metric("Sigma", str(round(sigma,2))+" F")
    with c4:
        if obs_high_final is not None:
            st.metric("Obs Floor", str(obs_high_final)+" F", delta="controlling" if obs_high_final >= consensus-0.1 else "not binding")
...
with st.form("log_form"):
    ...
    if st.form_submit_button("Log Settlement") and forecast is not None:
        entry = {"date": datetime.now().strftime("%Y-%m-%d"), "city": city, "actual": actual,
                 "forecast": round(forecast,1), "consensus": round(consensus,1),
                 "obs_high": obs_high_final, "error": round(actual-consensus,1)}
        history.append(entry); save_json(HISTORY_FILE, history[-300:])
...
if history:
    ...
    if wd:
        ...
        with hc2: st.metric("Model MAE", str(round(mae,2))+" F")
        with hc3: st.metric("Within 1 degree", str(round(w1*100,0))+"%")
