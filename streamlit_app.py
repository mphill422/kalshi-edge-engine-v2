# Kalshi High Temperature Model - V4.13
...
st.set_page_config(page_title='Kalshi High Temp V4.13', layout='wide')
st.title('Kalshi High Temperature Model - V4.13')
...
HEADERS = {'User-Agent': 'kalshi-temp-model/4.13', 'Accept': 'application/geo+json, application/json, text/html'}
...
def fetch_gfs_ensemble(lat, lon):
    """
    Fetch GFS ensemble daily max temps from Open-Meteo ensemble API.
    Returns (list_of_member_temps, ensemble_mean) or (None, None).
    """
    ...
    today_idx = next((i for i, t in enumerate(times) if t == today), None)
    if today_idx is None and times:
        today_idx = 0
    ...
    for key, vals in daily.items():
        if key == 'time':
            continue
        if isinstance(vals, list) and today_idx is not None and today_idx < len(vals):
            v = vals[today_idx]
            if v is not None:
                try:
